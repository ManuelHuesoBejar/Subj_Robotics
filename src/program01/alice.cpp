...
Alice::direct(const Pose& in, Node& out) {
    ...
    //Comprobamos que la posici�n final se encuentre dentro de los l�mites del espacio de trabajo del robot
    //Colocamos el origen de coordenadas en la base. An�lisis del peor caso:
    //A�adir una condicion en out.rz para que no se supere el giro maximo del servo?
...
Alice::inverse(const Node& in, Pose& out) {
    ...
    //Hay que a�adirle condiciones para limitar las rotaciones de los servos
    if ((d > l2 / 2 || d < l2 / 2))     {
        return 1;
    }
